...
# ✅ 【重要】package.jsonを先に作成してからBreezeインストール
echo "📝 package.jsonを事前作成中..."
...
# ✅ Breeze インストール（--pest追加でnpm installをスキップ）
echo "🎨 Breezeをインストール中..."
./vendor/bin/sail php artisan breeze:install vue --pest --dark --no-interaction

# ✅ package.jsonを再度上書き（念のため）
echo "📝 package.jsonを再確認中..."
cat > package.json <<'PACKAGE_JSON'
{
    "private": true,
    "type": "module",
